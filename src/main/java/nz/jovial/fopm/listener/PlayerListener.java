...
public class PlayerListener implements Listener
{
    ...
    public void onPlayerJoin(PlayerJoinEvent event)
    {
        ...
        event.joinMessage(LegacyComponentSerializer.legacySection().deserialize(ChatColor.DARK_GRAY + "[" + ChatColor.GREEN + "+" + ChatColor.DARK_GRAY + "] " + player.getName()));
        ...
        if (AdminList.isAdmin(player))
        {
            if (!AdminList.getAdmin(player).getIps().contains(player.getAddress().getHostString()))
            {
                ...
                Bukkit.broadcast(Component.text(ChatColor.RED + player.getName() + " has been flagged as an imposter!"));
            ...
            Bukkit.broadcast(LegacyComponentSerializer.legacySection().deserialize(ChatColor.YELLOW + player.getName() + Rank.getRank(player).getLoginMessage()));
    ...
    public void onPlayerLogin(PlayerLoginEvent event)
    {
        ...
        if (ban != null && !ban.isExpired())
        {
            ...
            event.disallow(PlayerLoginEvent.Result.KICK_OTHER, Component.text(ban.getKickMessage()));
        }
    }
    ...
    public void onPlayerDisconnect(PlayerQuitEvent event) {
        final Player player = event.getPlayer();
        event.quitMessage(LegacyComponentSerializer.legacySection().deserialize(ChatColor.DARK_GRAY + "[" + ChatColor.RED + "-" + ChatColor.DARK_GRAY + "] " + player.getName()));
    }
    ...
    public void onPlayerTeleport(PlayerTeleportEvent event)
    {
        ...
        {
            event.setCancelled(true); // Cancel teleport
            player.sendMessage(ChatColor.RED + "You must be at least " + plugin.wm.fl.getRank().getName() + " to be able to teleport to flatlands!");
        }
    }
