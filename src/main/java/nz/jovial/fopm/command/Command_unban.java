...
@CommandParameters(description = "Unbans a player or an IP", usage = "/<command> <username | ip>", source = SourceType.BOTH, rank = Rank.ADMIN)
public class Command_unban
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args)
    {
        ...
        for (Ban ban : BanManager.getBansByType(BanType.NORMAL))
        {
            ...
            {
                Bukkit.broadcast(Component.text(ChatColor.GREEN + sender.getName() + " - Unbanning " + ban.getName()));
        ...
        for (Ban ban : BanManager.getBansByType(BanType.IP))
        {
            ...
            {
                Bukkit.broadcast(Component.text(ChatColor.GREEN + sender.getName() + " - Unbanning IP: " + target));
                BanManager.removeBan(ban);
                return true;
            }
        }
