...
public class CommandLoader
{
    ...
    private static CommandMap getCommandMap()
    {
        if (cmap == null)
        {
            try
            {
                // In Paper 1.21, commandMap is still accessible via reflection but might be in a different place or require different access
                // However, Bukkit.getCommandMap() is available in newer API versions
                // If not available, we fall back to reflection
                try {
